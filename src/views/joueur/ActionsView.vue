    ...
    <div v-else>
      <article class="carte-joueur">
        ...
        <form v-else @submit.prevent="lecture.executer({ type: 'deplacer', id: destination })">
          <label for="destination-joueur">Destination<select id="destination-joueur" v-model="destination" required><option value="" disabled>Choisir un lieu</option><option v-for="lieu in lecture.lieux" :key="lieu.id" :value="lieu.id">{{ lieu.nom }}</option></select></label>
      ...
      <div class="grille-joueur"><ActionJoueur v-for="action in lecture.actionsChapitres" :key="`${action.type}-${action.id}`" :action="action" /></div>
