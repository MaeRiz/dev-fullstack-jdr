...
export const CLE_STOCKAGE = 'jdr.joueurs.v1';

const useJoueursStore = defineStore('joueurs', () => {
  // States
  const liste = ref([]);
  ...
  const parCampagne = computed(() => campagneId => liste.value.filter(joueur => joueur.campagneId === campagneId));
  ...
  return {
    liste, parId, parCampagne,
    ajouter, modifier, supprimer,
    lectureImpossible, enregistrer,
  };
});

export default useJoueursStore;
