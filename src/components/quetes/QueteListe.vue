...
<template>
  <ul>
    <li v-for="quete in quetes" :key="quete.id" :class="{ 'avec-ordre': ordre }">
      <OrdreControls
        v-if="ordre"
        :haut-desactive="ordre.findIndex((element) => element.id === quete.id) === 0"
        :bas-desactive="ordre.findIndex((element) => element.id === quete.id) === ordre.length - 1"
        :nom="quete.nom"
        @deplacer="emit('deplacer', quete.id, $event)"
      />
      <div class="contenu-quete">
        <h2>{{ quete.nom }}</h2>
        <p>Chapitre : {{ libelleChapitre(quete.chapitreId) }}</p>
        <p>
          <span class="badge" :class="quete.etat">{{ quete.etat }}</span>
        </p>
