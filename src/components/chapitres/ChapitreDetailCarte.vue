...
<template>
  <article class="panel panneau">
    ...
    <details>
      ...
      <p class="text-content texte">
        <strong>Commentaire MJ :</strong>
        {{ chapitre.commentaireMj || "Aucun." }}
      </p>
      <p class="texte">
      ...
      <p>
        ...
        {{ chapitre.recompensesObjets.map((objet) => `${libelle(objets, objet.objetId)} × ${objet.quantite}`).join(", ") || "Aucun" }}
      </p>
      <p>
    ...
    <p>
      <RouterLink
        :to="{
          ...
          query: { campagneId: chapitre.campagneId, chapitreId: chapitre.id },
        }"
        >Gérer les quêtes de ce chapitre</RouterLink
      >
    </p>
    <ol v-if="quetes.length">
      <li v-for="quete in quetes" :key="quete.id">
        <RouterLink
          :to="{
            name: 'mj-edition-quete',
            query: { chapitreId: chapitre.id, queteId: quete.id },
          }"
          >{{ quete.nom }}</RouterLink
        >
        — {{ quete.etat }}
      </li>
    </ol>
    ...
    <div v-if="suppression" class="suppression" role="group" :aria-label="`Confirmer la suppression de ${chapitre.nom}`">
      ...
      <div class="actions">
        <button class="btn danger" @click="emit('confirmer-suppression', chapitre)">Confirmer la suppression</button
        ><button class="btn secondary" @click="emit('annuler-suppression')">Annuler</button>
      </div>
    </div>
  </article>
</template>
